...
class FriendCheck
{
  ...
  private function standby_ed()
  {
    ...
    $this->check_fr_type();
    if(!empty($this->data)){
      $this->view('リクエスト受付中','href="../mypage/mypage.php?friend_type=new#friends_all"');
    }else{
      $this->standby_ing();
    }
  }
  ...
  //html 出力
  private function view($value, $link = 'id="go" rel="leanModal" href="#fr_standby"')
  {
    echo '
        <div class="add_friend">
        <button class="add_btn"><a class="click"'.$link.'><i class="fa fa-user-plus" style="color:white">'.$value.'</i>
        </a>
        </button>
        </div>
    ';
