...
class Friend_edit
{
	...
	*/
	...
	public function check_data()
	{
		...
		$comment1_type = $this->h($_POST['c_type1']);
		$comment1 = $this->h($_POST['comment1']);
		$comment2_type = $this->h($_POST['c_type2']);
		$comment2 = $this->h($_POST['comment2']);
		$comment3_type = $this->h($_POST['c_type3']);
	...
	{	
		try{
	    	...
	    	$fr_email = $data['sender_email']; //友達のemail
	    	$sql = $pdo->prepare('UPDATE friends SET comment1_type=?, comment1=?, comment2_type=?, comment2=?, comment3_type=?, comment3=?, comment4=?, relation=? WHERE sender_email=? AND accepter_email=?');
	    	$sql->execute(array($this->comment1_type, $this->comment1, $this->comment2_type, $this->comment2, $this->comment3_type, $this->comment3, $this->comment4, $this->relation,$my_email, $fr_email));
	    	$pdo = null;
	    	$this->header();
		}catch(PDOException $e){
	    	echo('Error'.$e->getMessage());
