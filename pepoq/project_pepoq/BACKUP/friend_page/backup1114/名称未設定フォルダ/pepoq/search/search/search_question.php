...
if ($_GET['search_word'] !== '') {
	...
	foreach ($result_question as $key => $value) {
		...
		$data[$key][0]['created'] = $time_and_date->getPostTimeDate($data[$key][0]['created']);
		$data[$key][] = get_personal_data($value['email']);
		if ($data[$key][0]['anonymity']) {
			$data[$key][1]['name'] = '匿名さん';
		}
			...
			<div class="question_format">
				...
				<div class="user_box">
					<div class="user_images f_f">
						<a href="../../friend_page/profile/profile.php?friend_id='.$value[1][0]['id'].'">
							<img src="../../mypage/profile/img/thumbnail/'.$value[1][0]['thumbnail'].'" style="width:60px;height:60px;">
						</a>
					</div>
					<div class="user_item f_f">
						<div class="user_name">
							<p><a href="../../friend_page/profile/profile.php?friend_id='.$value[1][0]['id'].'">'.$value[1][0]['name'].'</a></p>
						</div>
					</div>
