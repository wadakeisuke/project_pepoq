...
class FriendEdit
{
	...
	{
		include('../db_connect.php');
		$sql = $pdo->prepare('INSERT INTO new_friend(my_email, fr_email) VALUES (:my_email, :fr_email)');
		$sql->bindValue(':my_email', $_SESSION['mypage']['email']);
		$sql->bindValue(':fr_email', $friend_email);
		$sql->execute();
	}
	...
	{
		include('../db_connect.php');
		$sql = $pdo->prepare('DELETE FROM new_friend WHERE my_email = :my_email AND fr_email = :fr_email');
		$sql->bindValue(':my_email', $_SESSION['mypage']['email']);
		$sql->bindValue(':fr_email', $friend_email);
		$sql->execute();
	}
