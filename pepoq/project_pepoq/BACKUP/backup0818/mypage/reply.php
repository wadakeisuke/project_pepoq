...
$answer_id = h($_GET['answer_id']);
//$question_data = get_question_data($question_id);
//$questioner_data = get_personal_data($question_data['email']);

$answer_data_for_id = get_answer_data_for_id($answer_id);
$those_who_answer_data = get_personal_data($answer_data_for_id['email']);
//print_r($those_who_answer_data);
//exit;
/*
...
function get_answer_data_for_id($answer_id)
{
	include('../php/db_connect.php');
	$sql = $pdo->prepare('SELECT * FROM answer WHERE id = :answer_id');
	$sql->bindValue(':answer_id', $answer_id);
	$sql->execute();
	$answer_data_for_id = $sql->fetch(PDO::FETCH_ASSOC);
	return $answer_data_for_id;
}
			...
			<div style="clear:both;background-color:;height:50px;color:gray;">
				<ul>
					<li style="float:left;background-color:;margin-left:10px;">
						<a href="profile.php?friend_id='.$those_who_answer_data['id'].'"><img src="../img/thumbnail/group.jpg" style="height:40px;width:40px;"></a>
					</li>
					<li style="float:left;background-color:;margin-left:10px;">
						<a href="profile.php?friend_id='.$those_who_answer_data['id'].'">'.$those_who_answer_data[0]['name'].'</a>
					</li>					
				</ul>
			</div>
